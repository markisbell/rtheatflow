...
* ``POST /config/apply`` — swap the running engine onto a catalog network,
  ...
  never a process restart, SPEC §3.4).
* ``GET /config/active`` — metadata of the current configuration.

``POST /networks/import`` is deferred to M6 (user_networks scan + upload
validation); the catalog serves the committed library in M4.
"""
...
from .runtime import build_topology, get_app, status_payload
