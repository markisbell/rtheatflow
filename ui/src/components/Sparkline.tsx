interface Props {
  values: number[];
  overlay?: number[]; // optional dashed second series
  width?: number;
  height?: number;
  color?: string;
  fill?: string;
  overlayColor?: string;
  hourAxis?: boolean; // x = hours 0..24; otherwise unitless samples
  marker?: number;    // horizontal limit/setpoint line at +marker
...
/** A compact area/line chart (blueprint Sparkline port): daily profiles,
 *  load-duration curves, live traces. Handles negative values with a zero
 *  baseline. */
export default function Sparkline({
  ...
  const x = (i: number, n: number) => padL + (i / Math.max(1, n - 1)) * w;
  const y = (v: number) => pad + h - ((v - min) / span) * h;
  const y0 = y(0);
  const yDec = span < 10 ? 1 : 0;        // tick label decimals
  const yTicks = axisTicks(min, max);

  // step = sample-and-hold: each value holds until the next sample (the
  // natural look for discrete per-minute profiles); otherwise linear
  ...
  return (
    <svg width={fluid ? undefined : width} height={fluid ? undefined : height}
         ...
         role="img" aria-label="profile chart">
